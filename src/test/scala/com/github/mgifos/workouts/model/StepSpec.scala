...
class StepSpec extends FlatSpec with Matchers {
  ...
  "Step" should "parse correctly" in {
    ...
    Step.parse("- warmup: 5km") should be(
      WarmupStep(DistanceDuration(5, km)))
    ...
    Step.parse("- cooldown: 05:00") should be(
    ...
    Step.parse("- cooldown: 05:00 #cooldown run") should be(
      CooldownStep(TimeDuration(minutes = 5), None, "cooldown run"))

    Step.parse("- recover: 500m @z2 #recovery run") should be(
      RecoverStep(DistanceDuration(500, m), Some(HrZoneTarget(2)), "recovery run"))
  }
}
